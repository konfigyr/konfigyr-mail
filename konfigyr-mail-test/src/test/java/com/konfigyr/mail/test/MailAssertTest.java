...
class MailAssertTest {
	...
	void shouldAssertMailMessage() {
		MailAssert.assertThat(mail)
			...
			.hasRecipients(
				Recipient.to("recipient@konfigyr.com", "Recipient"),
	...
	void shouldFailToAssertMailMessage() {
		...
		assertThatExceptionOfType(AssertionError.class)
			.isThrownBy(() -> instance.hasAttributeSatisfying("missing-key", it -> {}))
			.withMessageContaining("Mail message should contain the attribute that satisfies the asserting conditions: missing-key");
	}

	@Test
	@DisplayName("should fail to assert null mail message")
	void shouldFailToAssertNullMailMessage() {
		assertThatExceptionOfType(AssertionError.class)
			.isThrownBy(() -> MailAssert.assertThat(null).hasSubject("subject"));

		assertThatExceptionOfType(AssertionError.class)
			.isThrownBy(() -> MailAssert.assertThat(null).hasTemplate("template"));

		assertThatExceptionOfType(AssertionError.class)
			.isThrownBy(() -> MailAssert.assertThat(null).sentBy(new Address("sender@konfigyr.com")));

		assertThatExceptionOfType(AssertionError.class)
			.isThrownBy(() -> MailAssert.assertThat(null).hasReplyTo(new Address("reply@konfigyr.com")));

		assertThatExceptionOfType(AssertionError.class)
			.isThrownBy(() -> MailAssert.assertThat(null).hasRecipients(Recipient.to("x@konfigyr.com")));

		assertThatExceptionOfType(AssertionError.class)
			.isThrownBy(() -> MailAssert.assertThat(null).containsRecipient(Recipient.to("x@konfigyr.com")));

		assertThatExceptionOfType(AssertionError.class)
			.isThrownBy(() -> MailAssert.assertThat(null).containsRecipients(Recipient.to("x@konfigyr.com")));

		assertThatExceptionOfType(AssertionError.class)
			.isThrownBy(() -> MailAssert.assertThat(null).hasEncoding(StandardCharsets.UTF_8));

		assertThatExceptionOfType(AssertionError.class)
			.isThrownBy(() -> MailAssert.assertThat(null).hasLocale(Locale.ENGLISH));

		assertThatExceptionOfType(AssertionError.class)
			.isThrownBy(() -> MailAssert.assertThat(null).hasAttribute("key", "value"));

		assertThatExceptionOfType(AssertionError.class)
			.isThrownBy(() -> MailAssert.assertThat(null).hasAttributeSatisfying("key", it -> {}));
	}
